...
public class BarterArea extends Sprite {
    ...
    public BarterArea(Owner owner, Resource res, Message sdt) {
        ...
        for (int i = 0; i < vert.length; i++) {
            ...
            bu = Math.min(vert[i].y, bu);
            bl = Math.min(vert[i].x, bl);
            bb = Math.max(vert[i].y, bb);
            br = Math.max(vert[i].x, br);
        }
        extramat = res.layer(Material.Res.class, fl & 3).get();
    ...
    void trace(MeshBuf buf, float x1, float y1, float x2, float y2) {
        ...
        while (true) {
            ...
            buf.new Face(lh, ll, nh);
            buf.new Face(ll, nl, nh);
            ll = nl;
            lh = nh;
            lx = nx;
            ly = ny;
    ...
    Rendered mkbound(int[] face, Coord3f[] vert) {
