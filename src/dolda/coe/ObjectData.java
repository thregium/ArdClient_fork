...
public interface ObjectData<T> {
    Map<Class<?>, ObjectData<?>> encoders = new HashMap<>();
    Map<Class, List<ObjectData>> scache = new HashMap<>();

    void encode(T obj, Map<Symbol, Object> buf);

    static <T> void register(Class<T> type, ObjectData<? super T> encoder) {
    ...
    static <T extends Serializable> ObjectData<T> serializer(Class<T> cl) {
    ...
    static Symbol s(String nm) {
        return (Symbol.get(nm));
    }
